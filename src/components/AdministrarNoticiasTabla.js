...
export default function AdministrarNoticiasTabla({ rows, handleClickBorrar, handleClickNotificar, handleClickEditar }) {
                ...
                <Table className={classes.table} size="small">
                    <TableHead>
                        <TableRow key={20} className="primary">
                            ...
                            <TableCell><strong>Fecha publicación</strong></TableCell>
                            <TableCell></TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody >
                        {rows.map(row => (
                            <TableRow key={row.id}>
                                ...
                                <TableCell>{`${row.titulo.substring(0, 200)}...`}</TableCell>
                                <TableCell>{`${row.descripcion.substring(0, 200)}...`}</TableCell>
                                <TableCell>{row.visible == 0 ? 'NO' : 'SI'}</TableCell>
                                <TableCell>{row.fechaPublicacion != null ? row.fechaPublicacion.substring(0, 10) : ''}</TableCell>
                                <TableCell component="th" scope="row">
